...
```{r Check cell population}
# check marker expression on cells
CytoExploreR::cyto_plot(gs,
                        select = 1,          
                        channels = c("CD3","CD8"),
                        parent = "root",
                        ylim = c(0,4))
...
#CytoExploreR::cyto_plot_gating_scheme(gs)

```

### 2.3) Defining cell populations
...
channels_clust <- fs[[1]]@parameters@data %>% dplyr::filter(desc %in% markers_clust)%>%dplyr::pull(name) %>% unname
...
-   **`MarkerComb`**: A factor describing which marker combination a specific cell has. It represents all mutually exclusive combinations (e.g., 4 markers → 2⁴ = 16 combinations).
...
The two modes are described in the figure below (`TRUE` = black, `FALSE` = grey):

<img src="figures/assignMarkerCombinations_modes.png" width="100%"/>

### 4.3) Calculating peptide-specificity

To calculate peptide-specificity, we need to add sample metadata to define cell population levels and stimulation conditions. Here we choose `resolution = "MarkerComb` to quantify at individual marker combination levels, but any column output from the previous function can be used (e.g. MarkerCount, Functionality, etc..)
...
The results can be plottet using ggplot. From this, we see that metaclusters 2 and 4 are the most HIV-1 specific clusters, and that metacluster 4 has a high frequency of triple positive IFNg+TNFa+CD107a+ cells:
...
result %>%
  ...
  ggplot(aes(x = cell_type,
             ...
             fill = MarkerComb))+
  facet_grid2(Stimulation~ID)+
  geom_bar(stat="identity")+
  theme_bw()+
  ylab("Frequency of HIV-1-Gag specific CD8 T cells")+
  xlab("CD8 metacluster")+
  scale_fill_manual(values = c("#DC050C","#FB8072","#1965B0","#7BAFDE","#882E72","#B17BA6","#FF7F00","#FDB462","#E7298A","#E78AC3","#33A02C", "#B2DF8A","#55A1B1","#8DD3C7","#A6761D"))
```
